...
#define   WIFI_PASSWORD           "senssun@2019" 
// #define   WIFI_SSID               "Zoujq-iphone11"
// #define   WIFI_PASSWORD           "zz123456" 
...
void xs_wifi_init(void)
{
    ...
    strcpy((char *)wifi_config.sta_config.ssid, g_app_data.wifi_ssid);
    strcpy((char *)wifi_config.sta_config.password, g_app_data.wifi_password);
    ...
    wifi_start();
    printf("wifi start connect wifi:%s,%s \r\n",wifi_config.sta_config.ssid,wifi_config.sta_config.password);
    /* Create task */
    task_def.name = "user_app";
    task_def.stacksize = OS_TASK_STACK_SIZE_APP;
    task_def.tpriority = OS_TASK_PRIORITY_APP;
    task_def.pthread = wifi_thread;
    wifi_task_id = osThreadCreate(&task_def, (void*)NULL);
